...
All 70 tools exposed by `nodriver-mcp`, with the exact parameters the server advertises over MCP.
...
- [Network & console](#network--console) (10)
...
| Parameter | Type | Required | Description |
|---|---|---|---|
| `action` | `start` \| `status` \| `stop` | — | "start" begins capturing in the background, "status" reports progress while it runs, "stop" ends it and reports what was saved. *(default: `start`)* |
| `url_pattern` | `string` | — | Which responses to keep, matched against the full URL — required for start. "*" matches any run of characters and "?" exactly one, as in Chrome's own interception patterns: "*/api/*", "*graphql*". A pattern without "*" matches anywhere in the URL. "*" keeps everything. |
| `out_dir` | `string` | — | Folder to write into, created if missing. Empty uses ~/.nodriver-mcp/captures/<timestamp>. |
| `resource_types` | array of `Document` \| `Stylesheet` \| `Image` \| `Media` \| `Font` \| `Script` \| `TextTrack` \| `XHR` \| `Fetch` \| `Prefetch` \| `EventSource` \| `WebSocket` \| `Manifest` \| `SignedExchange` \| `Ping` \| `CSPViolationReport` \| `Preflight` \| `FedCM` \| `Other` | — | Only these resource types, e.g. ["XHR", "Fetch"] for a page's own API calls. Omit for all. |
| `max_body_mb` | `integer` (min 1, max 150) | — | Skip a response whose Content-Length announces more than this many MB. A captured body is held in memory whole on its way to disk. *(default: `100`)* |
| `browser` | `string` | — | Which browser to act on. Each name is an independent Chrome, created on first use. Omit for the shared default; parallel agents each need their own name. *(default: `default`)* |
...
| Parameter | Type | Required | Description |
|---|---|---|---|
| `query` | `string` | yes | Text to look for, or a regular expression with regex=true. |
| `regex` | `boolean` | — | Treat query as a Python regular expression. *(default: `false`)* |
| `case_sensitive` | `boolean` | — | Match case exactly. Default false. *(default: `false`)* |
| `directory` | `string` | — | A capture_bodies folder to search. Empty uses the running capture, or the most recent one. |
| `max_results` | `integer` (min 1, max 500) | — | Stop after this many matches. *(default: `50`)* |
| `browser` | `string` | — | Which browser to act on. Each name is an independent Chrome, created on first use. Omit for the shared default; parallel agents each need their own name. *(default: `default`)* |
...
| Parameter | Type | Required | Description |
|---|---|---|---|
| `file_path` | `string` | yes | Where to write the .har file. |
| `include_bodies` | `boolean` | — | Embed response bodies: from capture_bodies' files where there is one, otherwise whatever Chrome still holds. Off gives headers and timing only. *(default: `true`)* |
| `include_preserved_requests` | `boolean` | — | Also export the previous 3 navigations' requests, not just the current page's. *(default: `true`)* |
| `url_filter` | `string` | — | Only requests whose URL contains this substring, e.g. "/api/". Empty exports all. |
| `browser` | `string` | — | Which browser to act on. Each name is an independent Chrome, created on first use. Omit for the shared default; parallel agents each need their own name. *(default: `default`)* |
...
| Parameter | Type | Required | Description |
|---|---|---|---|
| `checks` | array of `headers` \| `tls` \| `cookies` \| `cache` \| `domains` \| `issues` | — | Which parts to run: "headers" (security headers, CSP analysis, CORS on every recorded response, version disclosure), "tls" (protocol, cipher, certificate of every origin the page talked to), "cookies" (flags of every cookie sent to those origins), "cache" (responses a CDN or proxy could serve to the wrong user), "domains" (first and third parties the page talks to, and third-party code loaded without integrity checks) and "issues" (what Chrome itself flagged: CSP violations, mixed content, rejected cookies, CORS errors). Omit for all. |
| `browser` | `string` | — | Which browser to act on. Each name is an independent Chrome, created on first use. Omit for the shared default; parallel agents each need their own name. *(default: `default`)* |
...
| Parameter | Type | Required | Description |
|---|---|---|---|
| `reveal_values` | `boolean` | — | Print values that look like credentials in full (up to 120 characters). Default false shows their first characters and length only. *(default: `false`)* |
| `browser` | `string` | — | Which browser to act on. Each name is an independent Chrome, created on first use. Omit for the shared default; parallel agents each need their own name. *(default: `default`)* |
