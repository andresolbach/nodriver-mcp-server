...
All 57 tools exposed by `nodriver-mcp`, with the exact parameters the server advertises over MCP.
...
| Parameter | Type | Required | Description |
|---|---|---|---|
| `function` | `string` | yes | A JavaScript function expression, e.g. "() => document.title" or "(el) => el.innerText". It is invoked immediately and its return value is JSON-serialised back to you. Async functions are awaited, so "async () => (await fetch('/api/x')).json()" works. Return a value — console.log output is not captured. |
